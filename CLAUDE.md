...
- Don't default to the plainest possible implementation. Use thoughtful spacing, typography hierarchy, and subtle color/shadow choices — this is an internal business tool, not a wireframe, but it should still feel considered rather than bare-minimum. Look at existing styled components in the project (e.g. LoginPage) for the established visual language and stay consistent with it rather than inventing a new style each time.
  You have creative latitude here — exact colors, spacing values, and layout details don't need to be specified in every prompt. Use good judgment by default so this doesn't need correcting after the fact.
...
- Never read the contents of `.env` (use `.env.example` as the
  reference for what variables are expected).
- Writing new variable declarations to `.env`/`.env.example` is fine.
